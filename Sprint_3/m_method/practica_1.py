class BacteriaProducer:
    ...
    def __init__(self, max_bacteria):
        self.max_bacteria = max_bacteria  # !!!max>current!!!
        self.current_bacteria_count = BacteriaProducer.current_bacteria   # current !< 0
    ...
    def create(self):
        if self.current_bacteria_count == self.max_bacteria:
            return print('Нет места под новую бактерию')
        else:
            self.current_bacteria_count += 1
            return print(f'Добавлена одна бактерия. Бактерий в колонии: {self.current_bacteria_count}')
    ...
    def delete(self):
        ...
        if self.current_bacteria_count <= 0:
            return print(f'В популяции нет бактерий, удалять нечего')
        else:
            self.current_bacteria_count -= 1
            return print(f'Одна бактерия удалена. Бактерий в колонии: {self.current_bacteria_count}')
